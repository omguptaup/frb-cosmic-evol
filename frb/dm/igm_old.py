...
def f_ISM(z, cosmo=defs.frb_cosmo, return_rho = False,
              perturb_Mstar=None):
  """
  Calculate the cosmic fraction of baryons
  in diffuse gas phase based on our empirical
  ...
  rho_ISM = avg_rhoISM(z, cosmo=cosmo)
  if not return_rho:
   return (rho_ISM/rho_b).value
  else:
   return np.ones(z.shape[0]), rho_b*1*(1+z)**3

def f_star(z, cosmo=defs.frb_cosmo, return_rho = False,
              perturb_Mstar=None):
  """
  Calculate the cosmic fraction of baryons
  in diffuse gas phase based on our empirical
...
def avg_rhoMstar(z, remnants=True):
    ...
    z, flg_z = z_to_array(z)
    # Load
    stellar_mass_file = resource_filename('frb', os.path.join('data','IGM','stellarmass_md14.dat'))
