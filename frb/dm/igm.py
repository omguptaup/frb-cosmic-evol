...
def average_fHI(z, z_HIreion=5.8):
    """
    Average HI fraction
    1 = neutral
    0 = fully ionized
    ...
    H_file = resource_filename('frb', os.path.join('data','IGM','qhII.txt'))
    qHII = Table.read(H_file, format='ascii')
    
    first_H_ionized = np.where(qHII['Q_HII'] >= 1.)[0][0]
    z_HIreion = qHII['z'][first_H_ionized]
    z_HImax = 15
    
    fHI = np.zeros_like(z)
    #
    zion = (z > z_HIreion) & (z < z_HImax)
    fi_HII = interp1d(qHII['z'], qHII['Q_HII'])
    fHI[zion] = 1 - fi_HII(z[zion])
    # Return
    if flg_z:
        return fHI
    else:
...
def average_He_nume(z, z_HeIreion=7.):
    """
    Average number of electrons contributed by He as a function of redshift
    per He nucleus
    
    Following Finkelstein, D'Aloisio, Paardekooper et al. 2019
    for HI reionization, and assuming same for HeI
        https://arxiv.org/abs/1902.02792
    
    Following Kulkarni, Worseck & Hennawi 2018
    for HeII reionization
        https://arxiv.org/abs/1807.09774

    Args:
      z (float or ndarray): Redshift
      z_HeIreion (float, optional): Redshift where first 
          helium reionization ends.
    ...
    # Load Finkelstein Table
    H_file = resource_filename('frb', os.path.join('data','IGM','qhII.txt'))
    qHII = Table.read(H_file, format='ascii')
    # Fully He re-ionized
    first_He_ionized = np.where(qHeIII['Q_HeIII_18'] >= 1.)[0][0]
    z_HeIIreion = qHeIII['z'][first_He_ionized]
    ...
    fHeIII = np.zeros_like(z)
    
    # HeI ionized at HI reionization
    fHeII[z < z_HIreion] = 1.
    zion = (z > z_HIreion) & (z < z_HImax)
    fi_HeII = interp1d(qHII['z'], qHII['Q_HII'])
    fHeII[zion] = fi_HeII(z[zion])
    
    # HeII ionized at HeII reionization
    zion2 = (z > z_HeIIreion) & (z < z_HeIreion)
    fi_HeIII = interp1d(qHeIII['z'], qHeIII['Q_HeIII_18'])
    fHeIII[zion2] = fi_HeIII(z[zion2])
    fHeIII[z < z_HeIIreion] = 1
...
def f_diffuse(z, cosmo=defs.frb_cosmo, return_rho = False,
  ...
  # else:
  #  return np.ones(z.shape[0]), rho_b*1*(1+z)**3
...
def avg_rhoSFR(z):
    ...
    Based on MLE fit of Madau & Dickinson (2014) data for z < 4
    and, on nonlinear LSE for z >= 4 using data from Finkelstein+2022 and my own compilation
    ...
    """
    rho_SFR_unitless = np.zeros_like(z)
    z04 = z <= 4
    z414 = z>4
    rho_SFR_unitless[z04] = 0.0152 * (1+z[z04])**2.732 / (1 + ((1+z[z04])/3.004)**6.240)
    rho_SFR_unitless[z414] = 10**(-0.2571*z[z414] - 0.2749)
